...
nets = dict(
    ...
    jackpotcoin=math.Object(
        P2P_PREFIX='c0dbfafc'.decode('hex'), #stripped from jackpotcoind's main.cpp -> pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd };
        P2P_PORT=15371, #jackpotcoind 's p2p port
        ADDRESS_VERSION=43, #look again in the sourcecode in the file base58.h, and find the value of PUBKEY_ADDRESS.
        RPC_PORT=15372, #jackpotcoind 's rpc port
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'jackpotcoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 100000*30000000000 >> (height + 1)//5040,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=120, # one block generation time
        SYMBOL='JPC',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'jackpotcoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/jackpotcoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.jackpotcoin'), 'jackpotcoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://explorer.jackpotcoin.info/',
        ADDRESS_EXPLORER_URL_PREFIX='http://explorer.jackpotcoin.info/address/',
        TX_EXPLORER_URL_PREFIX='http://explorer.jackpotcoin.info/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1), #??
        DUMB_SCRYPT_DIFF=2**16, #??
        DUST_THRESHOLD=0.03e8, #??
    ),
